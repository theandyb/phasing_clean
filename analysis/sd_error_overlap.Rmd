...
df_pop_id <- data.frame(id = 1:1000, pop = rep(c("EUR", "AFR", "AMR", "EAS", "SAS"), each = 200))
```
...
df_err_pct %>%
  mutate(Beagle = s_b / s_b_total,
         ...
         SHAPEIT = s_s / s_s_total) %>%
  select(pop, Beagle, Eagle, SHAPEIT) %>%
  tbl_summary(by = pop, statistic = list(all_continuous() ~ "{mean} ({sd})"), digits = all_continuous() ~ 4)
```
...
df_err_pct %>% 
  mutate(Beagle = s_b / s_b_total,
         ...
         Beagle_SHAPEIT = s_b_s / s_b_total,
         Beagle_Eagle_SHAPEIT = s_b_e_s / s_b_total) %>%
  select(pop, starts_with("Beagle")) %>%
  tbl_summary(statistic = list(all_continuous() ~ "{mean} ({sd})"), digits = all_continuous() ~ 4, by = pop)
```


In Beagle, on average we observe that 40.5% of its switches are unique to Beagle, while on average 14% are shared with Eagle, 16.9% are shared with SHAPEIT, and 28.6% of Beagle's switches are shared with both Eagle and SHAPEIT.

### Eagle

```{r}
df_err_pct %>% 
  mutate(Eagle = s_e / s_e_total,
         Beagle_Eagle = s_b_e / s_e_total,
         Eagle_SHAPEIT = s_e_s / s_e_total,
         Beagle_Eagle_SHAPEIT = s_b_e_s / s_e_total) %>%
  select(contains("Eagle")) %>%
  tbl_summary(statistic = list(all_continuous() ~ "{mean} ({sd})"), digits = all_continuous() ~ 4)
...
df_err_pct %>% 
  mutate(Eagle = s_e / s_e_total,
         ...
         Eagle_SHAPEIT = s_e_s / s_e_total,
         Beagle_Eagle_SHAPEIT = s_b_e_s / s_e_total) %>%
  select(pop, contains("Eagle")) %>%
  tbl_summary(statistic = list(all_continuous() ~ "{mean} ({sd})"), digits = all_continuous() ~ 4, by = pop)
```

In Eagle, on average 60.4% of switches are unique to Eagle, 10.1% are shared with Beagle, 8.9% are shared with SHAPEIT, and 20.6% are shared with both Beagle and SHAPEIT.

### SHAPEIT

```{r}
df_err_pct %>% 
  mutate(SHAPEIT = s_s / s_s_total,
         Beagle_SHAPEIT = s_b_s / s_s_total,
         Eagle_SHAPEIT = s_e_s / s_s_total,
         Beagle_Eagle_SHAPEIT = s_b_e_s / s_s_total) %>%
  select(contains("SHAPEIT")) %>%
  tbl_summary(statistic = list(all_continuous() ~ "{mean} ({sd})"), digits = all_continuous() ~ 4)
...
df_err_pct %>% 
  mutate(SHAPEIT = s_s / s_s_total,
         ...
         Eagle_SHAPEIT = s_e_s / s_s_total,
         Beagle_Eagle_SHAPEIT = s_b_e_s / s_s_total) %>%
  select(pop, contains("SHAPEIT")) %>%
  tbl_summary(statistic = list(all_continuous() ~ "{mean} ({sd})"), digits = all_continuous() ~ 4, by = pop)
```

In SHAPEIT, on average 36.1% of switches are unique to SHAPEIT, 18.7% are shared with Beagle, 13.6% are shared with Eagle, and 31.6% are shared with both Beagle and Eagle.

## Flips

```{r}
df_err_pct %>%
  mutate(Beagle = f_b / f_b_total,
         Eagle = f_e / f_e_total,
         SHAPEIT = f_s / f_s_total) %>%
  select(pop, Beagle, Eagle, SHAPEIT) %>%
  pivot_longer(-pop, names_to = "alg", values_to = "prop") %>%
  ggplot(aes(x = alg, y = prop, colour = pop)) +
  geom_boxplot() +
  ...
  labs(color = "Population") +
  xlab("Method") +
  ylab("Proportion of Unique Flips")
...
```{r}
df_err_pct %>%
  mutate(Beagle = (f_b_total - f_b ) / f_b_total,
         Eagle = (f_e_total - f_e) / f_e_total,
         SHAPEIT = (f_s_total - f_s) / f_s_total) %>%
  select(pop, Beagle, Eagle, SHAPEIT) %>%
  pivot_longer(-pop, names_to = "alg", values_to = "prop") %>%
  ggplot(aes(x = alg, y = prop, colour = pop)) +
  geom_boxplot() +
  ...
  labs(color = "Population") +
  xlab("Method") +
  ylab("Proportion of Shared Flips")
...
df_err_pct %>% 
  mutate(Beagle = f_b / f_b_total,
         Beagle_Eagle = f_b_e / f_b_total,
         Beagle_SHAPEIT = f_b_s / f_b_total,
         Beagle_Eagle_SHAPEIT = f_b_e_s / f_b_total) %>%
  select(starts_with("Beagle")) %>%
  tbl_summary(statistic = list(all_continuous() ~ "{mean} ({sd})"), digits = all_continuous() ~ 4)
...
```{r}
df_err_pct %>% 
  mutate(Eagle = f_e / f_e_total,
         Beagle_Eagle = f_b_e / f_e_total,
         Eagle_SHAPEIT = f_e_s / f_e_total,
         Beagle_Eagle_SHAPEIT = f_b_e_s / f_e_total) %>%
  select(contains("Eagle")) %>%
  tbl_summary(statistic = list(all_continuous() ~ "{mean} ({sd})"), digits = all_continuous() ~ 4)
...
```{r}
df_err_pct %>% 
  mutate(SHAPEIT = f_s / f_s_total,
         Beagle_SHAPEIT = f_b_s / f_s_total,
         Eagle_SHAPEIT = f_e_s / f_s_total,
         Beagle_Eagle_SHAPEIT = f_b_e_s / f_s_total) %>%
  select(contains("SHAPEIT")) %>%
  tbl_summary(statistic = list(all_continuous() ~ "{mean} ({sd})"), digits = all_continuous() ~ 4)
...
Here, our denominator will be the total number of errors observed in a synthetic diploid across all three methods (i.e. count of unique switch and flip locations).
...
```{r}
df_pct %>%
  select(pop, Y_N_N_switch, N_Y_N_switch, N_N_Y_switch) %>%
  rename(Beagle = Y_N_N_switch,
         Eagle = N_Y_N_switch,
         SHAPEIT = N_N_Y_switch) %>%
  tbl_summary(statistic = list(all_continuous() ~ "{mean} ({sd})"), digits = all_continuous() ~ 4, by = pop)

df_pct %>%
  select(Y_N_N_switch, N_Y_N_switch, N_N_Y_switch) %>%
  rename(Beagle = Y_N_N_switch,
         Eagle = N_Y_N_switch,
         SHAPEIT = N_N_Y_switch) %>%
  tbl_summary(statistic = list(all_continuous() ~ "{mean} ({sd})"), digits = all_continuous() ~ 4)
...
```{r}
df_pct %>%
  select(contains("N_switch"), contains("Y_switch")) %>%
  tbl_summary(statistic = list(all_continuous() ~ "{mean} ({sd})"), digits = all_continuous() ~ 4)

df_pct %>%
  select(pop, contains("N_switch"), contains("Y_switch")) %>%
  tbl_summary(statistic = list(all_continuous() ~ "{mean} ({sd})"), digits = all_continuous() ~ 4, by = pop)
```
...
```{r}
df_pct %>%
  select(pop, Y_N_N_flip, N_Y_N_flip, N_N_Y_flip) %>%
  rename(Beagle = Y_N_N_flip,
         Eagle = N_Y_N_flip,
         SHAPEIT = N_N_Y_flip) %>%
  tbl_summary(statistic = list(all_continuous() ~ "{mean} ({sd})"), digits = all_continuous() ~ 4, by = pop)

df_pct %>%
  select(Y_N_N_flip, N_Y_N_flip, N_N_Y_flip) %>%
  rename(Beagle = Y_N_N_flip,
         Eagle = N_Y_N_flip,
         SHAPEIT = N_N_Y_flip) %>%
  tbl_summary(statistic = list(all_continuous() ~ "{mean} ({sd})"), digits = all_continuous() ~ 4)
...
```{r}
df_pct %>%
  select(contains("N_flip"), contains("Y_flip")) %>%
  tbl_summary(statistic = list(all_continuous() ~ "{mean} ({sd})"), digits = all_continuous() ~ 4)

df_pct %>%
  select(pop, contains("N_flip"), contains("Y_flip")) %>%
  tbl_summary(statistic = list(all_continuous() ~ "{mean} ({sd})"), digits = all_continuous() ~ 4, by = pop)
```
...
```{r}
df_pct %>%
  select(pop, Y_N_N_error, N_Y_N_error, N_N_Y_error) %>%
  rename(Beagle = Y_N_N_error,
         Eagle = N_Y_N_error,
         SHAPEIT = N_N_Y_error) %>%
  tbl_summary(statistic = list(all_continuous() ~ "{mean} ({sd})"), digits = all_continuous() ~ 4, by = pop)

df_pct %>%
  select(Y_N_N_error, N_Y_N_error, N_N_Y_error) %>%
  rename(Beagle = Y_N_N_error,
         Eagle = N_Y_N_error,
         SHAPEIT = N_N_Y_error) %>%
  tbl_summary(statistic = list(all_continuous() ~ "{mean} ({sd})"), digits = all_continuous() ~ 4)
...
```{r}
df_pct %>%
  select(contains("N_error"), contains("Y_error")) %>%
  tbl_summary(statistic = list(all_continuous() ~ "{mean} ({sd})"), digits = all_continuous() ~ 4)

df_pct %>%
  select(pop, contains("N_error"), contains("Y_error")) %>%
  tbl_summary(statistic = list(all_continuous() ~ "{mean} ({sd})"), digits = all_continuous() ~ 4, by = pop)
```
...
I believe that this material should be enough to write up a paragraph or two for the draft paper. Namely, I'd like to have a summary of

1. How often are the same errors observed across methods
2. To what extent are errors unique to a particular method

With regards to the latter point, I'd like to dig in more into the differences across methods: is their anything regarding the genomic context around errors unique to a particular method that would allow us to explain the difference in performance between the methods?
...
# Alternate: distributions of percents in bins

Instead of returning a single number per bin for each superpopulation, let's instead get values for each individual x bin (there's so much unused memory on the cluster anyways).

```{r}
get_errors_bin <- function(id, bin_size, het_loc_dir, beagle_switch_dir, eagle_switch_dir, shapeit_switch_dir, df_freq){
  
  final_df <- annotate_het_list(id, 
                                het_loc_dir, 
                                beagle_switch_dir, 
                                eagle_switch_dir, 
                                shapeit_switch_dir, 
...
get_errors_bin_df <- function(ids, bin_size, het_loc_dir, beagle_switch_dir, eagle_switch_dir, shapeit_switch_dir, df_freq){
  results <- vector(mode = "list", length = length(ids))
  for(i in 1:length(ids)){
    results[[i]] <- get_errors_bin(ids[i], bin_size, het_loc_dir, beagle_switch_dir, eagle_switch_dir, shapeit_switch_dir, df_freq)
  }
  return(bind_rows(results))
}
```
...
```{r}
df_all <- get_errors_bin_df(1:1000, 1e6, het_loc_dir, beagle_switch_dir, eagle_switch_dir, shapeit_switch_dir, df_freq)
df_all <- left_join(df_all, df_pop_id, by = "id")
```

### All

Let us first look at the average number of switches and flips per individual in each of the MB bins:

```{r}
df_all %>%
  group_by(bin_id) %>%
  summarise(Beagle = mean(beagle_s),
            Eagle = mean(eagle_s),
            SHAPEIT = mean(shapeit_s)) %>%
  pivot_longer(-bin_id, names_to = "method", values_to = "switches") %>% 
  ggplot(aes(x = bin_id, y = switches, colour = method)) +
  geom_point() +
  geom_line() +
  theme_classic() +
  xlab("Bin (MB)") +
  ylab("Mean Switches per Bin") +
  scale_color_manual(values = cbPalette)
...
df_all %>%
  mutate(ten_bin = ceiling(bin_id / 10)) %>%
  filter(ten_bin == 7) %>%
  group_by(bin_id) %>%
  summarise(Beagle = mean(beagle_s),
            Eagle = mean(eagle_s),
  ...
  ylab("Mean Switches per Bin") +
  scale_color_manual(values = cbPalette) +
  ggtitle("MB Bins in 10MB Bin 7")


df_all %>%
  group_by(bin_id) %>%
  summarise(Beagle = mean(beagle_f),
            Eagle = mean(eagle_f),
            SHAPEIT = mean(shapeit_f)) %>%
  pivot_longer(-bin_id, names_to = "method", values_to = "switches") %>% 
  ggplot(aes(x = bin_id, y = switches, colour = method)) +
  geom_point() +
  geom_line() +
...
Similarly, we can also look at average rates per het:
...
Let's take a look at what our data looks like at the MB scale. First, let's look at our EUR synthetic diploids:

```{r}
df_all %>%
  filter(pop == "EUR") %>%
  mutate(beagle_het = beagle_s / hets,
         eagle_het = eagle_s / hets,
         shapeit_het = shapeit_s/ hets) %>%
  group_by(bin_id) %>%
  summarise(beagle = mean(beagle_het),
            eagle = mean(eagle_het),
            shapeit = mean(shapeit_het)) %>%
  pivot_longer(-bin_id, names_to = "method", values_to = "switch_per_het") %>%
  ggplot(aes(x = bin_id, y = switch_per_het, color = method)) +
  geom_point() +
  geom_line() +
  xlab("Bin MB") +
  ylab("Mean Switches per Het") +
  theme_classic() +
  scale_color_manual(values = cbPalette)

df_all %>%
  filter(pop == "EUR") %>%
  mutate(beagle_het = beagle_f / hets,
         eagle_het = eagle_f / hets,
         shapeit_het = shapeit_f/ hets) %>%
  group_by(bin_id) %>%
  summarise(beagle = mean(beagle_het),
            eagle = mean(eagle_het),
            shapeit = mean(shapeit_het)) %>%
  ...
  geom_line() +
  xlab("Bin MB") +
  ylab("Mean Flips per Het") +
  theme_classic() +
  scale_color_manual(values = cbPalette)
```

We see that for our EUR sds, there are several bins with elevated switches/het values for Eagle; these regions are also higher for the other two methods, but not to the same extent to which they are in EUR. With our data frame, we can evaluate across individuals what this trend looks like for Eagle:
...
df_all %>%
  ...
  mutate(beagle_het = beagle_s / hets,
         eagle_het = eagle_s / hets,
         shapeit_het = shapeit_s/ hets) %>%
  ggplot(aes(x = bin_id, y = eagle_het, group = id)) +
  geom_point() +
  geom_line() +
  theme_classic() +
  xlab("Bin (MB)") +
  ylab("Switches per het (Eagle)")
...
df_all %>%
  ...
  mutate(beagle_het = beagle_s / hets,
         eagle_het = eagle_s / hets,
         shapeit_het = shapeit_s/ hets) %>%
  ggplot(aes(x = factor(bin_id), y = eagle_het)) +
  geom_boxplot() + 
  theme_classic() +
  xlab("Bin (MB)") +
  ylab("Switches per het (Eagle)")
...
df_all %>%
  ...
  mutate(beagle_het = beagle_s / hets,
         eagle_het = eagle_s / hets,
         shapeit_het = shapeit_s/ hets) %>%
  select(bin_id, id, ends_with("_het")) %>%
  pivot_longer(ends_with("_het"), names_to = "method", values_to = "switches") %>%
  ggplot(aes(x = factor(bin_id), y = switches, color = method)) +
  geom_boxplot() + 
  theme_classic() +
  xlab("Bin (MB)") +
  ylab("Switches per het (Eagle)") +
...
And for flips, we can evaluate what the distribution of flips in each bin looks like for SHAPEIT:
...
df_all %>%
  ...
  mutate(beagle_het = beagle_f / hets,
         eagle_het = eagle_f / hets,
         shapeit_het = shapeit_f/ hets) %>%
  ggplot(aes(x = bin_id, y = shapeit_het, group = id)) +
  geom_point() +
  geom_line() +
  theme_classic() +
  xlab("Bin (MB)") +
  ylab("Flips per het (SHAPEIT)")
...
df_all %>%
  ...
  mutate(beagle_het = beagle_f / hets,
         eagle_het = eagle_f / hets,
         shapeit_het = shapeit_f/ hets) %>%
  ggplot(aes(x = factor(bin_id), y = shapeit_het)) +
  geom_boxplot() +
  theme_classic() +
  xlab("Bin (MB)") +
  ylab("Flips per het (SHAPEIT)")
...
Hard to see anything with these figures; the outliers for switches look to be individuals with perhaps few (1) heterozygous positions in a bin, and the switch happened to occur at that position. Let's instead look at the raw counts per bin for each SD:

```{r}
df_all %>%
  filter(pop == "EUR") %>%
  ggplot(aes(x = bin_id, y = eagle_s, group = id)) +
  geom_point() +
  geom_line() +
  theme_classic() + 
  xlab("Bin (MB)") +
  ylab("Switches per bin (Eagle)")
```

Now, this might be quite the mess, but let's see if we can produce this figure with all three methods at once:

```{r}
df_all %>%
  filter(pop == "EUR") %>%
  select(bin_id, id, ends_with("_s")) %>%
  pivot_longer(ends_with("_s"), names_to = "method", values_to = "switches") %>%
  ggplot(aes(x = bin_id, y = switches, group = id, color = method)) +
  geom_point() +
  geom_line()
```
...
Let's zoom out and see if we can get a better glimpse at chromosome-wide patterns

```{r}
#df_eur <- get_errors_bin_df(1:200, 1e7, het_loc_dir, beagle_switch_dir, eagle_switch_dir, shapeit_switch_dir, df_freq)

df_all <- get_errors_bin_df(1:1000, 1e7, het_loc_dir, beagle_switch_dir, eagle_switch_dir, shapeit_switch_dir, df_freq)
...
First let's look at the switch and flip rate across all individuals in 10MB bins. We'll start with the raw counts for switches and flips.
...
df_all %>%
  ...
  ggplot(aes(x = bin_id, y = switches, colour = method)) +
    ...
    geom_line() +
    xlab("Bin (10MB)") +
    ylab("Average Number of Switches") +
    theme_classic() +
    scale_color_manual(values = cbPalette) +
  labs(color = "Method")

df_all %>%
  mutate(e_b = eagle_s - beagle_s,
         e_s = eagle_s - shapeit_s) %>%
  group_by(bin_id) %>%
  summarise(Eagle_Beagle = mean(e_b),
            Eagle_SHAPEIT = mean(e_s)) %>%
  pivot_longer(-bin_id, names_to = "method", values_to = "switches") %>%
  ggplot(aes(x = bin_id, y = switches, colour = method)) +
    geom_point() +
    geom_line() +
    xlab("Bin (10MB)") +
    ylab("Average Eagle Switch Difference") +
    theme_classic() +
    scale_color_manual(values = cbPalette) +
  labs(color = "Method")
...
We'll then look at the rate per heterozygous site (perhaps on a log scale?):
...
df_all %>%
  ...
  ggplot(aes(x = bin_id, y = switches, colour = method)) +
    ...
    geom_line() +
    xlab("Bin (10MB)") +
    ylab("Average Number of Switches per Het.") +
    theme_classic() +
    scale_color_manual(values = cbPalette) +
  labs(color = "Method")
```
...
df_all %>%
  ...
  mutate(beagle_het = beagle_s / hets,
         eagle_het = eagle_s / hets,
         shapeit_het = shapeit_s/ hets) %>%
  ggplot(aes(x = bin_id, y = eagle_het, group = bin_id)) +
  geom_boxplot()
...
df_all %>%
  ...
  mutate(beagle_het = beagle_s / hets,
         eagle_het = eagle_s / hets,
         shapeit_het = shapeit_s/ hets) %>%
  select(bin_id, ends_with("_het")) %>%
  pivot_longer(-bin_id, names_to = "method", values_to = "switch_rate") %>%
  ggplot(aes(x = factor(bin_id), y = switch_rate, color = method)) +
  geom_boxplot() +
  theme_classic() +
  xlab("Bin (10MB)") +
  ylab("Switch rate") +
