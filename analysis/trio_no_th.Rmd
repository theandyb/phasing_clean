...
cross_df_plots <- function(df, df_th, metric = "other"){
  ...
  p3 <- df_tmp %>%
    ...
    labs(color="Population") +
    scale_color_manual(values = cbPalette) +
    theme(legend.position.inside = c(.85, .2))
...
Again, we observe the same qualitative ranking of the methods: SHAPEIT generally introduces more flips than the other two methods, while more most probands we observe more flips in Beagle than in Eagle. Now, if we plot the error rates for each method acrss the datasets, we observe:
