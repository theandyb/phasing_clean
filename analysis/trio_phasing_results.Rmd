...
df_switch_trio %>%
  mutate(Beagle = (beagle_switch_th / n_other_beagle),
         Eagle = (eagle_switch_th / n_other_eagle),
         SHAPEIT = (shapeit_switch_th / n_other_shapeit)) %>%
  select(SUPER, Eagle, Beagle, SHAPEIT) %>%
  pivot_longer(-SUPER, names_to = "Method", values_to = "enrich") %>%
  ggplot(aes(x = enrich, color = SUPER)) +
    geom_density() +
    ylab("Frac. switches at TH") +
    scale_color_manual(values = cbPalette) +
    theme_classic()

df_switch_trio %>%
  mutate(Beagle = (beagle_flip_th / n_flip_beagle),
         Eagle = (eagle_flip_th / n_flip_eagle),
         SHAPEIT = (shapeit_flip_th / n_flip_shapeit)) %>%
  select(SUPER, Eagle, Beagle, SHAPEIT) %>%
  pivot_longer(-SUPER, names_to = "Method", values_to = "enrich") %>%
  ggplot(aes(x = enrich, color = SUPER)) +
    geom_density() +
    ylab("Frac. flips at TH") +
    scale_color_manual(values = cbPalette) +
    theme_classic()

df_switch_trio %>%
  mutate(Beagle = (beagle_switch_th / n_other_beagle) / (n_t_het / n_hets),
         Eagle = (eagle_switch_th / n_other_eagle) / (n_t_het / n_hets),
         SHAPEIT = (shapeit_switch_th / n_other_shapeit) / (n_t_het / n_hets)) %>%
  select(SUPER, Eagle, Beagle, SHAPEIT) %>%
  pivot_longer(-SUPER, names_to = "Method", values_to = "enrich") %>%
...
df_switch_trio %>%
  mutate(Beagle = (beagle_flip_th / n_flip_beagle) / (n_t_het / n_hets),
         Eagle = (eagle_flip_th / n_flip_eagle) / (n_t_het / n_hets),
         SHAPEIT = (shapeit_flip_th / n_flip_shapeit) / (n_t_het / n_hets)) %>%
  select(SUPER, Eagle, Beagle, SHAPEIT) %>%
  pivot_longer(-SUPER, names_to = "Method", values_to = "enrich") %>%
  ggplot(aes(x = Method, y = enrich, color = SUPER)) +
    geom_boxplot() +
    ylab("Frac. flips at TH / Frac. Het. at TH") +
    scale_color_manual(values = cbPalette) +
    theme_classic() +
  geom_hline(yintercept = 1)
```
