...
for(i in 1:22){
  df_het[[i]] <- read_tsv(paste0(config_obj$base_dir, "/output/trio_phase_", i, "/no_th/het_pos_count.tsv"),
...
What range of values do we observe in the autosomes?
...
Additionally, I would like to test the null hypothesis that the error rates are the same across chromosomes for each method:

```{r}
mod_fit <- df_total_noth %>%
  filter(chrom < 23) %>%
  {lmerTest::lmer(Beagle ~ chrom + (1|pair_id), data = .)}

df_anova <- anova(mod_fit) %>% broom::tidy() %>% mutate(method = "Beagle")

mod_fit <- df_total_noth %>%
  filter(chrom < 23) %>%
  {lmerTest::lmer(Eagle ~ chrom + (1|pair_id), data = .)}

df_anova <- bind_rows(df_anova, {anova(mod_fit) %>% broom::tidy() %>% mutate(method = "Eagle")})

mod_fit <- df_total_noth %>%
  filter(chrom < 23) %>%
  {lmerTest::lmer(SHAPEIT ~ chrom + (1|pair_id), data = .)}

df_anova <- bind_rows(df_anova, {anova(mod_fit) %>% broom::tidy() %>% mutate(method = "SHAPEIT")})
...
# Beagle
autosomes <- df_switches %>%  filter(chrom < 23) %>% pull(Beagle)
...
```{r}
# Beagle
autosomes <- df_flips %>% filter(chrom < 23) %>% pull(Beagle)
chrX <- df_flips %>% filter(chrom == 23) %>% pull(Beagle)
t.test(autosomes, chrX) %>% broom::tidy()

# Eagle
autosomes <- df_flips %>% filter(chrom < 23) %>% pull(Eagle)
chrX <- df_flips %>% filter(chrom == 23) %>% pull(Eagle)
t.test(autosomes, chrX) %>% broom::tidy()

# SHAPEIT
autosomes <- df_flips %>% filter(chrom < 23) %>% pull(SHAPEIT)
chrX <- df_flips %>% filter(chrom == 23) %>% pull(SHAPEIT)
t.test(autosomes, chrX) %>% broom::tidy()
```
