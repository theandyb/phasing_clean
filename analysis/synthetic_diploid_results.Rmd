...
#' Theme for Scientific Manuscripts
#' 
#' @param base_size Numeric. Base font size for the plot (default is 12 for standard manuscripts).
#' @param base_family Character. Font family to use (default is "sans" which maps to Arial/Helvetica).
#' 
theme_manuscript <- function(base_size = 12, base_family = "sans") {
  theme_classic(base_size = base_size, base_family = base_family) %+replace% 
    theme(
      # Ensure titles and labels are large and readable
      plot.title = element_text(size = rel(1.2), face = "bold", hjust = 0.5, margin = margin(b = 10)),
      plot.subtitle = element_text(size = rel(1.0), face = "plain", hjust = 0.5, margin = margin(b = 10)),
      ...
      strip.text = element_text(size = rel(0.9), face = "bold", margin = margin(t = 5, b = 5)),
      
      # Tweak plot margins so nothing gets cut off
...
df_vcftools_sub <- df_vcftools %>%
  ...
  mutate(total_rate = total_errors / n_het) %>%
  mutate(switch_rate = switches / n_het,
         flip_rate = flips / n_het) %>%
  select(sample_id, method, switches, flips, total_errors, n_het, switch_rate, flip_rate, total_rate)

df_vcftools_sub$pop <- rep(c("EUR", "AFR", "AMR", "EAS", "SAS"), each = 600)
...
# Violin plots of switch rates by method with white boxplot overlay
df_switch_noRare %>% # rename methods to Beagle, Eagle, SHAPEIT
  mutate(method = recode(method,
                         beagle = "Beagle",
                         eagle = "Eagle",
                         shapeit = "SHAPEIT")) %>%
  ggplot(aes(x = method, y = switch_rate, fill = method)) +
  geom_violin(width = 1) +
  ...
  scale_fill_manual(values = ggpubfigs::friendly_pal("vibrant_seven"))
```
...
# Violin plots of flip rates by method with white boxplot overlay
df_switch_noRare %>%
  mutate(method = recode(method,
                         beagle = "Beagle",
                         eagle = "Eagle",
                         shapeit = "SHAPEIT")) %>%
  ggplot(aes(x = method, y = flip_rate, fill = method)) +
  geom_violin(width = 1) +
  geom_boxplot(width=0.1, color="grey", alpha=0.2) +
  theme_manuscript(base_size=14) +
  labs(x = "Method",
...
#### Boxplots by population of switch and flip rates
...
df_vcftools %>%
  ...
  select(pair_id, pop, Beagle, Eagle, SHAPEIT) %>%
  pivot_longer(Beagle:SHAPEIT, names_to = "method", values_to = "switch_rate") %>%
  ggplot(aes(x = method, y = switch_rate, colour = pop)) +
  geom_boxplot() +
  theme_manuscript(base_size = 14) +
  scale_color_manual(values = okabe_ito_palette) +
  xlab("Method") +
  ylab("Switch Rate \nPer Heterozygous Site") +
  labs(color = "Population") +
  theme(
    legend.title = element_text(size = 14), # Makes title larger
    legend.text = element_text(size = 12)    # Makes labels larger
  )

# Use population as x axis and method as color
df_vcftools %>%
  mutate(Beagle = n_other_beagle / n_hets,
         Eagle = n_other_eagle / n_hets, 
         SHAPEIT = n_other_shapeit / n_hets) %>%
  select(pair_id, pop, Beagle, Eagle, SHAPEIT) %>%
  pivot_longer(Beagle:SHAPEIT, names_to = "method", values_to = "switch_rate") %>%
  ggplot(aes(x = pop, y = switch_rate, colour = method)) +
  geom_boxplot() +
  theme_manuscript(base_size = 14) +
  scale_color_manual(values = okabe_ito_palette) +
  xlab("Population") +
...
df_vcftools %>%
  ...
  select(pair_id, pop, Beagle, Eagle, SHAPEIT) %>%
  pivot_longer(Beagle:SHAPEIT, names_to = "method", values_to = "flip_rate") %>%
  ggplot(aes(x = method, y = flip_rate, colour = pop)) +
  geom_boxplot() +
  theme_manuscript(base_size = 14) +
  scale_color_manual(values = okabe_ito_palette) +
  xlab("Method") +
...
df_vcftools %>%
  ...
  select(pair_id, pop, Beagle, Eagle, SHAPEIT) %>%
  pivot_longer(Beagle:SHAPEIT, names_to = "method", values_to = "flip_rate") %>%
  ggplot(aes(x = pop, y = flip_rate, colour = method)) +
  geom_boxplot() +
  theme_manuscript(base_size = 14) +
  scale_color_manual(values = okabe_ito_palette) +
  xlab("Population") +
...
df_vcftools %>%
  ...
  select(pop, Beagle, Eagle, SHAPEIT) %>%
  pivot_longer(-pop, names_to = "method", values_to = "enrich") %>%
  ggplot(aes(x = pop, y = enrich, colour = method)) +
  geom_boxplot() +
  ggtitle("Enrichment of Errors at CpG Sites") +
  xlab("Population") +
  ylab("Enrichment") +
...
maf_prop_df %>%
  ...
  select(id, SP, Eagle, Beagle, SHAPEIT) %>%
  pivot_longer(Eagle:SHAPEIT, names_to = "method", values_to = "enrich") %>%
  ggplot(aes(x = SP, y = enrich, color = method)) +
  geom_boxplot() +
  theme_manuscript(base_size = 14) +
  ggtitle("Enrichment of Switches at Rare Variants") + 
  xlab("Population") +
