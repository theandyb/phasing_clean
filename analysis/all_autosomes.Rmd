...
theme_linedraw_blank <- function(base_size = 11, base_family = "", base_line_size = base_size/22, base_rect_size = base_size/22) {
  theme_linedraw(
    base_size = base_size,
...
theme_set(theme_linedraw_blank())
...
```{r}
# df_2 <- all_summary(1:602, "output/trio_phase_1/het_loc/annotated/")
# df_2 <- df_2 %>%
#   left_join({df_subj_rel %>% select(SUPER, sex, id2) %>% rename(id=id2, pop = SUPER)})

df_1 <- load_premade_summary("output/trio_phase_1/no_th/") %>% 
  select(-motherID, -fatherID, -id2, -SAMPLE_NAME)
df_2 <- load_premade_summary("output/trio_phase_2/no_th/") %>% 
  select(-motherID, -fatherID, -id2, -SAMPLE_NAME)
df_3 <- load_premade_summary("output/trio_phase_3/no_th/") %>% 
  select(-motherID, -fatherID, -id2, -SAMPLE_NAME)
df_4 <- load_premade_summary("output/trio_phase_4/no_th/") %>% 
  select(-motherID, -fatherID, -id2, -SAMPLE_NAME)
df_5 <- load_premade_summary("output/trio_phase_5/no_th/") %>% 
  select(-motherID, -fatherID, -id2, -SAMPLE_NAME)
df_6 <- load_premade_summary("output/trio_phase_6/no_th/") %>% 
  select(-motherID, -fatherID, -id2, -SAMPLE_NAME)
df_7 <- load_premade_summary("output/trio_phase_7/no_th/") %>% 
  select(-motherID, -fatherID, -id2, -SAMPLE_NAME)
df_8 <- load_premade_summary("output/trio_phase_8/no_th/") %>% 
  select(-motherID, -fatherID, -id2, -SAMPLE_NAME)
df_9 <- load_premade_summary("output/trio_phase_9/no_th/") %>% 
  select(-motherID, -fatherID, -id2, -SAMPLE_NAME)
df_10 <- load_premade_summary("output/trio_phase_10/no_th/") %>% 
  select(-motherID, -fatherID, -id2, -SAMPLE_NAME)
df_11 <- load_premade_summary("output/trio_phase_11/no_th/") %>% 
  select(-motherID, -fatherID, -id2, -SAMPLE_NAME)
df_12 <- load_premade_summary("output/trio_phase_12/no_th/") %>% 
  select(-motherID, -fatherID, -id2, -SAMPLE_NAME)
df_13 <- load_premade_summary("output/trio_phase_13/no_th/") %>% 
  select(-motherID, -fatherID, -id2, -SAMPLE_NAME)
df_14 <- load_premade_summary("output/trio_phase_14/no_th/") %>% 
  select(-motherID, -fatherID, -id2, -SAMPLE_NAME)
df_15 <- load_premade_summary("output/trio_phase_15/no_th/") %>% 
  select(-motherID, -fatherID, -id2, -SAMPLE_NAME)
df_16 <- load_premade_summary("output/trio_phase_16/no_th/") %>% 
  select(-motherID, -fatherID, -id2, -SAMPLE_NAME)
df_17 <- load_premade_summary("output/trio_phase_17/no_th/") %>% 
  select(-motherID, -fatherID, -id2, -SAMPLE_NAME)
df_18 <- load_premade_summary("output/trio_phase_18/no_th/") %>% 
  select(-motherID, -fatherID, -id2, -SAMPLE_NAME)
df_19 <- load_premade_summary("output/trio_phase_19/no_th/") %>% 
  select(-motherID, -fatherID, -id2, -SAMPLE_NAME)
df_20 <- load_premade_summary("output/trio_phase_20/no_th/") %>% 
  select(-motherID, -fatherID, -id2, -SAMPLE_NAME)
df_21 <- load_premade_summary("output/trio_phase_21/no_th/") %>% 
  select(-motherID, -fatherID, -id2, -SAMPLE_NAME)
df_22 <- load_premade_summary("output/trio_phase_22/no_th/") %>% 
  select(-motherID, -fatherID, -id2, -SAMPLE_NAME)
...
df_x <- load_premade_summary("output/switch_errors/") %>% 
  rename(SUPER = pop, POPULATION = subpop) %>%
  select(-size_mb)
df_x$chrom <- "X"
df_x$sex <- 1
df_x <- df_x[,colnames(df_22)]
...
plot_rr_window <- function(chrom, bin_size = 1e6){
  ...
  df_rr <- df_rr %>%
    mutate(bin = ceiling(begin / bin_size),
           width = end - begin) %>%
    group_by(bin) %>%
    summarize(rr= sum(rr * width) / sum(width))
...
```{r}
for(i in c(1:22, "X")){
  print(plot_rr_window(i) + geom_line() + ggtitle(paste0("Chromosome ", i)))
}
```
...
```{r}
for(i in c(1:22, "X")){
  print(plot_rr_window(i, bin_size = 1e5) + geom_line() + ggtitle(paste0("Chromosome ", i)))
}
```
...
```{r}
for(i in c(1:22, "X")){
  print(plot_rr_window(i, bin_size = 1e4) + geom_line() + ggtitle(paste0("Chromosome ", i)))
}
```
...
#   all_counts(1:602, paste0("output/trio_phase_", i, "/no_th/het_loc/"), df_subj_rel = df_subj_rel, bin_size = 1e6)
# }
# stopImplicitCluster()
# 
# for(i in 1:22){
